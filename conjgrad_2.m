function [xs, is, err] = conjgrad_1( Afunc, b, x0, maxiters, miniters, Mdiag )
...
for i = 1:maxiters
    ...
    err = (val - prevval)/val;
    if i > testgap && prevval < 0 && err < tolerance*testgap && i >= miniters
        fprintf('relative error is %f at %d CG steps\n', err, i)
        break;
    end 
end

    %errnorm = norm(Afunc(x)-b)/norm(Afunc(x0)-b);
    %fprintf('norm err = %f.\n', errnorm)
