...
% During each epoch, a double mini-batch is used to compute the
...
if jacket
    ...
    %GPUmat version:
    %mones = @(varargin) ones(varargin{:}, GPUdouble);
    %mzeros = @(varargin) zeros(varargin{:}, GPUdouble);
    %conv = @GPUdouble;
    %norm = @(x) sqrt(sum(x.*x));
    
    mrandn = @grandn;
else
    %use doubles (this can make cpu code go faster):
...
if hybridmode
    ...
    %store = @double; %cache activities on the cpu
else
    store = @double;
end
...
%pack all the parameters into a double vector for easy manipulation
    function M = pack(W,b)
...
intest = double(intest);
outtest = double(outtest);
...
for epoch = epoch:maxepoch
    ...
    %         hohj = 1;
    %     end
    [chs, iterses, err] = conjgrad_2( @(V)-computeBV(V), grad, ch, ceil(maxiters), ceil(miniters), precon );
    %[chs, iterses, err] = conjgrad_1( @(V)-computeBV(V), grad, ch, ceil(maxiters), ceil(miniters), precon );
    
    %[chs, iterses, err, v, valbk] = eigCG_1( @(V)-computeBV(V), grad, ch, ceil(maxiters), ceil(miniters), precon );
    ...
    if jud < tau1
        p = alphak^2 * ch;
    elseif jud >= tau1 && jud <= tau2
        p = alphak^2*ch + alphak * v;
    else
        p = alphak * v;
    end
    ...
    %% Note that here P is our update. --- Heeryerate
